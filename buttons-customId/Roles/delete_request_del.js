...
module.exports = async (bot, interaction, args) => {
    ...
    await interaction.channel.send({ content: `\`[DELETE-REQUEST]\` Модератор ${interaction.member} [ID: ${interaction.member.id}] удалил запрос от пользователя <@${req_db.DiscordID}> [ID: ${req_db.DiscordID}]\n\nЗапрос был рассмотрен и удалён за ${time(Date.now()-req_db.created_at)}` })
