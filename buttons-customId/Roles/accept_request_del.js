...
module.exports = async (bot, interaction, args) => {
    ...
    await interaction.channel.send({ content: `\`[CANCEL-REQUEST]\` Модератор ${interaction.member} [ID: ${interaction.member.id}] принял запрос от пользователя <@${req_db.DiscordID}> [ID: ${req_db.DiscordID}]\n\nЗапрос был рассмотрен и одобрен за ${time(Date.now()-req_db.created_at)}` })

    // const message_req = await bot.channels.cache.get("959064921621028874").messages.fetch("955780888539586570")

    // await message_req.reactions.removeAll()
    // await message_req.react("❌")

    await bot.channels.cache.get("959064921621028874").send({ content: `<@${req_db.DiscordID}>, модератор ${interaction.member} [ID: ${interaction.member.id}] принял Ваш запрос на снятие роли` })

    const member = await bot.guilds.cache.get("527799726557364237").members.cache.get(req_db.DiscordID)

    await giveroles.map((hz) => {
        if(member.roles.cache.get(hz)) {
            member.roles.remove(hz)
        }
    })
