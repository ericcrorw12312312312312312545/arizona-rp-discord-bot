...
module.exports = async (bot, interaction, args) => {
    ...
    await bot.channels.cache.get("959064921621028874").send({ content: `<@${req_db.DiscordID}>, модератор ${interaction.member} [ID: ${interaction.member.id}] принял Ваш запрос на получение роли` })

    const member = await bot.guilds.cache.get("527799726557364237").members.cache.get(req_db.DiscordID)

    const filter_roles = giveroles.filter(x => x != args)

    await member.roles.add(args)

    await filter_roles.map((hz) => member.roles.remove(hz))
